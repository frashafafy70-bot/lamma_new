...
class InheritanceCalculatorPage extends StatefulWidget {
  ...
  @override
  State<InheritanceCalculatorPage> createState() =>
      _InheritanceCalculatorPageState();
}
...
class _InheritanceCalculatorPageState extends State<InheritanceCalculatorPage> {
  ...
  void _calculateAdvancedInheritance() {
    ...
    double totalAgriSahms =
        (_totalFeddans * 576) + (_totalQirats * 24) + _totalSahms;
    ...
    int effectiveFullBros =
        _fullBrothers + _missingFullBros + _pregnantFullBros;
    int effectiveFullSisters = _fullSisters + _hermaphroditeFullBros;

    int effectiveConsBrothers =
        _consanguineBrothers + _missingConsBros + _pregnantConsBros;
    int effectiveConsSisters = _consanguineSisters + _hermaphroditeConsBros;

    int effectiveUterineSiblings = _uterineSiblings +
        _hermaphroditeUterineBros +
        _missingUterineBros +
        _pregnantUterineBros;

    int totalDhawiArham = _paternalAunts +
        _paternalAuntsOffspring +
        _maternalUncles +
        _maternalUnclesOffspring +
        _maternalAunts +
        _maternalAuntsOffspring;
    ...
    if (_deceasedSons > 0 || _deceasedDaughters > 0) {
      ...
      bool hypHasBranch = hypSons > 0 ||
          hypDaughters > 0 ||
          _grandsons > 0 ||
          _granddaughters > 0;
      int hypTotalSiblings = effectiveFullBros +
          effectiveFullSisters +
          effectiveConsBrothers +
          effectiveConsSisters +
          effectiveUterineSiblings;
      ...
      if (_mother == 1) {
        hypMotherAmount = abstractEstate *
            ((hypHasBranch || hypTotalSiblings >= 2) ? (1 / 6) : (1 / 3));
      }
      ...
      double hypFixed = hypWivesAmount +
          hypHusbandAmount +
          hypMotherAmount +
          hypFatherAmount +
          hypMaternalGrandmotherAmount +
          hypPaternalGrandmotherAmount;
      ...
      double rawObligatoryWill = (_deceasedSons * hypOneSonShare) +
          (_deceasedDaughters * hypOneDaughterShare);
      ...
      if (rawObligatoryWill > maxAllowedWill) {
        ...
        exp.writeln(
            '- تم حساب النصيب الافتراضي للأحفاد ووجد أنه يتجاوز ثلث التركة، فتم رده إلى (الثلث) قانوناً وشرعاً.');
      } else {
        obligatoryWillAmount = rawObligatoryWill;
        exp.writeln(
            '- تم استخراج النصيب الافتراضي للأحفاد (أبناء الفرع المتوفى) باعتبار أصلهم حياً.');
      }
      exp.writeln('');
    }
    ...
    bool hasBranch = effectiveSons > 0 ||
        effectiveDaughters > 0 ||
        _grandsons > 0 ||
        _granddaughters > 0;
    bool hasMaleBranch = effectiveSons > 0 || _grandsons > 0;
    int totalSiblings = effectiveFullBros +
        effectiveFullSisters +
        effectiveConsBrothers +
        effectiveConsSisters +
        effectiveUterineSiblings;
    ...
    bool areAllSiblingsBlocked =
        effectiveSons > 0 || _grandsons > 0 || _father == 1;
    bool areConsanguineBlocked = areAllSiblingsBlocked ||
        effectiveFullBros > 0 ||
        effectiveFullSisters >= 2;
    bool areUterineBlocked =
        areAllSiblingsBlocked || _grandfather == 1 || hasBranch;

    bool areNephewsBlocked = areAllSiblingsBlocked ||
        effectiveFullBros > 0 ||
        effectiveConsBrothers > 0 ||
        _grandfather == 1;
    bool areUnclesBlocked = areNephewsBlocked || _nephews > 0;
    bool areCousinsBlocked = areUnclesBlocked || _paternalUncles > 0;

    bool hasAnyAsaba = hasMaleBranch ||
        _father == 1 ||
        _grandfather == 1 ||
        effectiveFullBros > 0 ||
        effectiveConsBrothers > 0 ||
        ...
        _paternalUncles > 0 ||
        _cousins > 0;
    bool hasRaddTarget = effectiveDaughters > 0 ||
        _granddaughters > 0 ||
        _mother == 1 ||
        _paternalGrandmother == 1 ||
        _maternalGrandmother == 1 ||
        effectiveFullSisters > 0 ||
        effectiveConsSisters > 0 ||
        effectiveUterineSiblings > 0;
    ...
    String getBreakdown(double fraction) {
      ...
      if (_totalMeters > 0) {
        parts.add(
            '• عقارات ومباني: ${(fraction * _totalMeters).toStringAsFixed(2)} متر مربع');
      }
      if (totalAgriSahms > 0) {
    ...
    void addResult(String heir, String status, String share, double amount,
        {String? reason}) {
    ...
    if (_pregnancies > 0 ||
        _missingSons > 0 ||
        _hermaphroditeSons > 0 ||
        _pregnantFullBros > 0 ||
        _missingFullBros > 0 ||
        _hermaphroditeFullBros > 0 ||
        _pregnantConsBros > 0 ||
        _missingConsBros > 0 ||
        _hermaphroditeConsBros > 0 ||
        _pregnantUterineBros > 0 ||
        _missingUterineBros > 0 ||
        _hermaphroditeUterineBros > 0) {
      exp.writeln(
          '⚠️ ثانياً: قواعد الاحتياط الفقهي للحالات الخاصة (الغائب والخنثى والحمل)');
      exp.writeln(
          '- القاعدة العامة: يُعامل الحمل والغائب بتقدير الذكورة (لحجز النصيب الأكبر)، ويُعامل الخنثى المشكل بتقدير الأنوثة (النصيب الأقل المتيقن).');
      exp.writeln(
          '- الإخوة لأم (إن وجدوا بحالة احتياط): الذكر والأنثى فيهم سواء في القسمة.');
      exp.writeln('');
    }
    ...
    if (_isDeceasedMale && _wives > 0) {
      ...
      addResult(
          'الزوجة (لكل واحدة)', 'فرض', hasBranch ? '1/8' : '1/4', wivesAmount);
    }

    if (_mother == 1) {
      double share = (hasBranch || hasSiblingGroup) ? (1 / 6) : (1 / 3);
      motherAmount = estate * share;
      addResult('الأم', 'فرض', (hasBranch || hasSiblingGroup) ? '1/6' : '1/3',
          motherAmount);
    } else {
      ...
      if (_paternalGrandmother == 1) {
        if (isPaternalGrandmotherBlocked) {
          ...
          addResult('الجدة لأب', 'محجوبة', '0', 0.0,
              reason: 'بسبب وجود $reason');
        } else {
    ...
    if (effectiveUterineSiblings > 0) {
      if (areUterineBlocked) {
        addResult('الإخوة لأم', 'محجوبون', '0', 0.0,
            reason: 'وجود فرع وارث أو أصل مذكر');
      } else {
        ...
        if (_uterineSiblings > 0) {
          addResult(
              'ولد أم (لكل واحد)',
              'فرض',
              effectiveUterineSiblings == 1 ? '1/6' : '1/3',
              uterineSiblingsAmount);
        }
        if (_hermaphroditeUterineBros > 0) {
          addResult(
              ...
              effectiveUterineSiblings == 1 ? '1/6' : '1/3',
              uterineSiblingsAmount,
              reason: 'الذكر والأنثى فيه سواء');
        }
        if (_missingUterineBros > 0) {
          addResult('أخ لأم (غائب/مفقود)', 'موقوف', 'نصيب فرض',
              uterineSiblingsAmount * _missingUterineBros,
              reason: 'يُوقف لحين عودته');
        }
        if (_pregnantUterineBros > 0) {
          addResult('حمل (في أخ لأم)', 'موقوف', 'نصيب فرض',
              uterineSiblingsAmount * _pregnantUterineBros,
              reason: 'يُوقف احتياطاً لحين الولادة');
        }
      }
    }

    double totalFixedDistributed = (_wives * wivesAmount) +
        husbandAmount +
        motherAmount +
        maternalGrandmotherAmount +
        paternalGrandmotherAmount +
        fatherAmount +
        grandfatherAmount +
        (effectiveUterineSiblings * uterineSiblingsAmount);
    ...
    if (effectiveSons > 0) {
      ...
      if (_daughters > 0) {
        addResult('البنت (لكل واحدة)', 'عصبة بالغير', 'باقي للذكر مثل الحظين',
            daughterShare);
      }
      if (_hermaphroditeSons > 0) {
        addResult('ابن (خنثى مشكل)', 'فرض الأقل', 'نصيب أنثى',
            daughterShare * _hermaphroditeSons,
            reason: 'احتياطاً يُعامل كأنثى');
      }
      if (_sons > 0) {
        addResult('الابن (لكل واحد)', 'عصبة بالنفس', 'باقي للذكر مثل الحظين',
            sonShare);
      }
      if (_missingSons > 0) {
        addResult('الابن المفقود/الغائب', 'موقوف', 'نصيب كامل',
            sonShare * _missingSons,
            reason: 'يُوقف لحين عودته');
      }
      if (_pregnancies > 0) {
        addResult('الحمل المستكن (فرع)', 'موقوف', 'تقدير ذكر',
            sonShare * _pregnancies,
            reason: 'يُوقف احتياطاً لحين الولادة');
      }
      ...
      if (_daughters > 0) {
        addResult('البنت (لكل واحدة)', 'فرض',
            effectiveDaughters == 1 ? '1/2' : '2/3', daughterShare);
      }
      if (_hermaphroditeSons > 0) {
        addResult('ابن (خنثى مشكل)', 'فرض الأقل',
            effectiveDaughters == 1 ? '1/2' : '2/3', daughterShare,
            reason: 'يُعامل كأنثى احتياطاً');
      }
    ...
    if (_grandsons > 0 || _granddaughters > 0) {
      if (isGrandsonBlocked) {
        if (_grandsons > 0) {
          addResult('ابن الابن', 'محجوب', '0', 0.0,
              reason: 'وجود الفرع المذكر (أو الحمل/الغائب)');
        }
        if (_granddaughters > 0) {
          addResult('بنت الابن', 'محجوبة', '0', 0.0,
              reason: 'وجود الفرع الأعلى');
        }
      } else {
    ...
    if (!hasMaleBranch) {
      if (_father == 1) {
        ...
        addResult('الأب', 'فرض + عصبة',
            hasBranch ? '1/6 + باقي' : 'الكل تعصيباً', fatherAmount);
        residue = 0;
      } else if (_grandfather == 1 && !isGrandfatherBlocked) {
        grandfatherAmount += residue;
        addResult('الجد لأب', 'فرض + عصبة',
            hasBranch ? '1/6 + باقي' : 'الكل تعصيباً', grandfatherAmount);
        residue = 0;
      }
    }
    ...
    if (residue > 0) {
      if (areAllSiblingsBlocked) {
        if (_fullBrothers > 0) {
          addResult('الأخ الشقيق', 'محجوب', '0', 0.0,
              reason: 'مُحجب بالفرع المذكر المتيقن أو الحمل');
        }
        if (_fullSisters > 0) {
          addResult('الأخت الشقيقة', 'محجوبة', '0', 0.0,
              reason: 'وجود حاجب أقرب');
        }
        if (_hermaphroditeFullBros > 0) {
          addResult('أخ شقيق (خنثى)', 'محجوب', '0', 0.0,
              reason: 'وجود حاجب أقرب');
        }
        if (_missingFullBros > 0) {
          addResult('أخ شقيق (غائب)', 'محجوب', '0', 0.0,
              reason: 'وجود حاجب أقرب');
        }
        if (_pregnantFullBros > 0) {
          addResult('حمل (في أخ شقيق)', 'محجوب', '0', 0.0,
              reason: 'وجود حاجب أقرب');
        }
      } else {
        if (effectiveFullBros > 0 || effectiveFullSisters > 0) {
          ...
          double partValue = residue / totalParts;
          if (_fullBrothers > 0) {
            addResult('الأخ الشقيق (لكل واحد)', 'عصبة بالنفس', 'باقي تعصيباً',
                partValue * 2);
          }
          if (_missingFullBros > 0) {
            addResult('أخ شقيق (غائب)', 'موقوف', 'نصيب ذكر',
                partValue * 2 * _missingFullBros,
                reason: 'يُوقف لحين عودته');
          }
          if (_pregnantFullBros > 0) {
            addResult('حمل (أخ شقيق)', 'موقوف', 'تقدير ذكر',
                partValue * 2 * _pregnantFullBros,
                reason: 'يُوقف احتياطاً لحين الولادة');
          }
          if (_fullSisters > 0) {
            addResult('الأخت الشقيقة (لكل واحدة)', 'عصبة بالغير',
                'باقي تعصيباً', partValue);
          }
          if (_hermaphroditeFullBros > 0) {
            addResult(
                'أخ شقيق (خنثى)', 'عصبة بالغير', 'باقي تعصيباً', partValue,
                reason: 'يُعامل كأنثى احتياطاً');
          }
          residue = 0;
        }
    ...
    if (residue > 0) {
      if (areConsanguineBlocked) {
        addResult('الإخوة لأب', 'محجوبون', '0', 0.0,
            reason: 'وجود حاجب أقرب درجة');
      } else {
        double totalParts =
            (effectiveConsBrothers * 2.0) + effectiveConsSisters;
        double partValue = residue / totalParts;
        if (_consanguineBrothers > 0) {
          addResult('الأخ لأب (لكل واحد)', 'عصبة', 'باقي', partValue * 2);
        }
        if (_missingConsBros > 0) {
          addResult('أخ لأب (غائب)', 'موقوف', 'نصيب ذكر',
              partValue * 2 * _missingConsBros,
              reason: 'يُوقف لحين عودته');
        }
        if (_pregnantConsBros > 0) {
          addResult('حمل (أخ لأب)', 'موقوف', 'تقدير ذكر',
              partValue * 2 * _pregnantConsBros,
              reason: 'يُوقف احتياطاً لحين الولادة');
        }
        ...
        if (_hermaphroditeConsBros > 0) {
          addResult('أخ لأب (خنثى)', 'عصبة بالغير', 'باقي', partValue,
              reason: 'يُعامل كأنثى احتياطاً');
        }
        residue = 0;
      }
    ...
    if (_nephews > 0) {
      if (areNephewsBlocked) {
        addResult('أبناء الإخوة', 'محجوبون', '0', 0.0,
            reason: 'وجود إخوة أو فروع مذكرة');
      } else if (residue > 0) {
        double amountPerNephew = residue / _nephews;
        addResult('ابن الأخ (لكل واحد)', 'عصبة بالنفس', 'باقي تعصيباً',
            amountPerNephew);
        residue = 0;
      }
    }
    ...
    if (_paternalUncles > 0) {
      if (areUnclesBlocked) {
        addResult('الأعمام', 'محجوبون', '0', 0.0,
            reason: 'وجود حاجب أقرب درجة');
      } else if (residue > 0) {
        double amountPerUncle = residue / _paternalUncles;
        addResult(
            'العم (لكل واحد)', 'عصبة بالنفس', 'باقي تعصيباً', amountPerUncle);
        residue = 0;
      }
    }
    ...
    if (_cousins > 0) {
      if (areCousinsBlocked) {
        addResult('أبناء الأعمام', 'محجوبون', '0', 0.0,
            reason: 'وجود حاجب أقرب درجة');
      } else if (residue > 0) {
        double amountPerCousin = residue / _cousins;
        addResult('ابن العم (لكل واحد)', 'عصبة بالنفس', 'باقي تعصيباً',
            amountPerCousin);
        residue = 0;
      }
    }
    ...
    if (residue > 0) {
      if (hasRaddTarget) {
        addResult('الرد الشرعي', 'قاعدة الرد',
            'يرد الباقي لأصحاب الفروض نسبياً', residue);
      } else if (totalDhawiArham > 0) {
        if (areDhawiArhamBlocked) {
          addResult('ذوو الأرحام', 'محجوبون', '0', 0.0,
              reason: 'لوجود أصحاب فروض أو عصبات');
        } else {
          double amountPerArham = residue / totalDhawiArham;
          if (_paternalAunts > 0) {
            addResult('العمات (لكل واحدة)', 'توريث الأرحام', 'الباقي بالتساوي',
                amountPerArham);
          }
          if (_paternalAuntsOffspring > 0) {
            addResult('أبناء العمات (لكل واحد)', 'توريث الأرحام',
                'الباقي بالتساوي', amountPerArham);
          }
          if (_maternalUncles > 0) {
            addResult('الأخوال (لكل واحد)', 'توريث الأرحام', 'الباقي بالتساوي',
                amountPerArham);
          }
          if (_maternalUnclesOffspring > 0) {
            addResult('أبناء الأخوال (لكل واحد)', 'توريث الأرحام',
                'الباقي بالتساوي', amountPerArham);
          }
          if (_maternalAunts > 0) {
            addResult('الخالات (لكل واحدة)', 'توريث الأرحام', 'الباقي بالتساوي',
                amountPerArham);
          }
          if (_maternalAuntsOffspring > 0) {
            addResult('أبناء الخالات (لكل واحد)', 'توريث الأرحام',
                'الباقي بالتساوي', amountPerArham);
          }
          residue = 0;
        }
  ...
  Future<void> _saveToArchive() async {
    if (_results.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('برجاء حساب التركة أولاً قبل الحفظ ⚠️')));
      return;
    }
    ...
    final name = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          ...
          children: [
            Icon(Icons.cloud_upload_rounded, color: primaryNavy),
            const SizedBox(width: 8),
            const Text('حفظ في أرشيف الموكلين',
          ...
          children: [
            TextField(
                ...
                decoration: InputDecoration(
                    labelText: 'اسم الموكل / رقم القضية',
            ...
            onPressed: () {
              if (clientNameController.text.trim().isEmpty) {
                ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(
                    content: Text('برجاء إدخال اسم الموكل ⚠️'),
    ...
    try {
      ...
      await FirebaseFirestore.instance
          .collection('inheritance_archive')
          .add(archiveData);
      if (!context.mounted) {
        return;
      }
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('تم الحفظ بنجاح! ☁️✅'), backgroundColor: Colors.green));
    } catch (e) {
      if (!context.mounted) {
        return;
      }
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(textDirection: TextDirection.rtl, children: [
          Icon(Icons.lightbulb, color: goldAccent),
          const SizedBox(width: 8),
          const Text('شرح التأصيل الشرعي',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18))
        ]),
        content: SizedBox(
            width: double.maxFinite,
            child: SingleChildScrollView(
                child: Text(_explanationText,
      ...
      pw.MultiPage(
        ...
        header: (pw.Context context) {
          return pw.Container(
              ...
              child: pw.Column(children: [
                pw.Text('مكتب العدالة للمحاماة والاستشارات القانونية',
                    style: pw.TextStyle(
                        color: PdfColors.white,
                        fontSize: 22,
                        font: arabicBold)),
                pw.SizedBox(height: 5),
                pw.Text('إعداد الأستاذ / محمود المواريث - المحامي',
                    style:
                        pw.TextStyle(color: PdfColors.amber300, fontSize: 16))
              ]));
        },
        footer: (pw.Context context) {
          return pw.Container(
              ...
              alignment: pw.Alignment.center,
              child: pw.Text(
                  'تم استخراج هذا التقرير آلياً عبر محرك المواريث الذكي (منصة لمة) - صفحة ${context.pageNumber} من ${context.pagesCount}',
        ...
        build: (pw.Context context) {
          return [
            pw.Row(
                ...
                children: [
                  pw.Text('تقرير الفرز الشرعي وتقسيم الأعيان',
                      style: pw.TextStyle(
                          fontSize: 18,
                          font: arabicBold,
                          color: PdfColors.black)),
                  pw.Text(
                      'تاريخ التقرير: ${DateTime.now().year}/${DateTime.now().month}/${DateTime.now().day}',
            ...
            pw.TableHelper.fromTextArray(
              headers: [
                'النصيب المفرز من الأعيان والأصول',
                'النسبة الشرعية',
                'الحالة القانونية',
                'الوارث'
              ],
              data: _results.map((res) {
                ...
                String percentStr = isBlocked
                    ? '0%'
                    : '${(res['fraction'] * 100).toStringAsFixed(2)}%';
                String breakdown =
                    isBlocked ? 'محجوب (لا شيء)' : res['breakdown'];
                return [
                  breakdown,
                  percentStr,
                  "${res['status']}\n${res['reason'] ?? res['share']}",
                  res['heir']
                ];
              }).toList(),
              headerStyle: pw.TextStyle(
                  font: arabicBold, color: PdfColors.white, fontSize: 10),
              headerDecoration:
            ...
            pw.Text('💡 شرح التأصيل الشرعي وحيثيات القسمة:',
                style: pw.TextStyle(
                    fontSize: 14, font: arabicBold, color: PdfColors.blue900)),
            pw.Divider(color: PdfColors.blueGrey200),
            pw.SizedBox(height: 10),
            pw.Text(_explanationText,
                style: pw.TextStyle(
                    font: arabicFont,
                    fontSize: 11,
                    lineSpacing: 2,
            ...
            pw.Divider(),
            pw.Text('الإقرار القانوني للمصادقة:',
                style: pw.TextStyle(
                    font: arabicBold, fontSize: 12, color: PdfColors.red900)),
            pw.Paragraph(
                text: _legalDeclaration,
                style: pw.TextStyle(
                    font: arabicFont, fontSize: 10, lineSpacing: 1.5)),
            pw.SizedBox(height: 40),
            pw.Row(
                mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                children: [
                  pw.Text('توقيع الموكل / الورثة: ___________________',
                      style: pw.TextStyle(font: arabicBold)),
                  pw.Text('توقيع المحامي المعتمد: ___________________',
                      style: pw.TextStyle(font: arabicBold))
                ])
          ];
        },
  ...
  Future<void> _printPdf() async {
    try {
      ...
      await Printing.layoutPdf(
          ...
          name: 'تقرير_مواريث_${DateTime.now().millisecondsSinceEpoch}.pdf');
    } catch (error) {
      if (!context.mounted) {
        return;
      }
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('عذراً، فشل فتح شاشة الطباعة ⚠️\n$error'),
  ...
  Future<void> _downloadPdf() async {
    try {
      ...
      await Printing.sharePdf(
          bytes: bytes,
          filename:
              'تقرير_مواريث_${DateTime.now().millisecondsSinceEpoch}.pdf');
    } catch (error) {
      if (!context.mounted) {
        return;
      }
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('عذراً، فشل تحميل الملف للجهاز ⚠️\n$error'),
          backgroundColor: Colors.red.shade800));
    }
  }
        ...
        children: [
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  _buildSectionCardHeader('بيانات التركة والأصول',
                      Icons.account_balance_wallet_rounded, primaryNavy),
                  const Divider(height: 20),
                  _buildAssetInput('النقدية والسيولة بالبنوك (ج.م)',
                      Icons.attach_money_rounded, _cashCtrl),
                  _buildAssetInput('مساحة العقارات والمباني (بالمتر المربع)',
                      Icons.apartment_rounded, _buildingMetersCtrl),
                  const Padding(
                    padding: EdgeInsets.symmetric(vertical: 8.0),
                    child: Text('مساحة الأراضي الزراعية:',
                        style: TextStyle(
                            fontSize: 13,
                            fontWeight: FontWeight.bold,
                            color: Colors.green,
                  ...
                  Row(
                    children: [
                      Expanded(
                          child: _buildAssetInput('فدان',
                              Icons.landscape_rounded, _agriFeddansCtrl)),
                      const SizedBox(width: 8),
                      Expanded(
                          child: _buildAssetInput(
                              'قيراط', Icons.grass_rounded, _agriQiratsCtrl)),
                      const SizedBox(width: 8),
                      Expanded(
                          child: _buildAssetInput(
                              'سهم', Icons.eco_rounded, _agriSahmsCtrl)),
                    ],
                  ),
                  _buildAssetInput('الذهب والمجوهرات (بالجرام)',
                      Icons.diamond_rounded, _goldCtrl),
                  _buildAssetInput('سيارات وأصول أخرى (تقييم مالي ج.م)',
                      Icons.directions_car_rounded, _otherCashCtrl),
                ],
              ),
            ),
          ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildSectionCardHeader(
                            'الوصية الواجبة (القانون المصري)',
                            Icons.gavel_rounded,
                            Colors.amber.shade800),
                        _buildUniformCounter(
                            'أبناء ذكور متوفين قبله',
                            _deceasedSons,
                            () => setState(() => _deceasedSons++),
                            () => setState(() => _deceasedSons--)),
                        _buildUniformCounter(
                            'بنات إناث متوفيات قبله',
                            _deceasedDaughters,
                            () => setState(() => _deceasedDaughters++),
                            () => setState(() => _deceasedDaughters--)),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
                  _buildSectionCardHeader('أصحاب الفروض (الأزواج)',
                      Icons.people_alt_rounded, primaryNavy),
                  if (_isDeceasedMale)
                    _buildUniformCounter(
                        'الزوجات',
                        _wives,
                    ...
                    _buildUniformCounter(
                        ...
                        max: 1),
                  const Divider(height: 25),
                  _buildSectionCardHeader('الفروع (الأحياء وقت الوفاة)',
                      Icons.child_care_rounded, primaryNavy),
                  _buildUniformCounter(
                      'الأبناء الصلبيين',
                      _sons,
                      () => setState(() => _sons++),
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildSectionCardHeader('احتياط الفروع (موقوف/مُجمد)',
                            Icons.shield_rounded, Colors.blueGrey.shade800),
                        _buildUniformCounter(
                            'حمل مستكن (يُوقف له ذكر احتياطاً)',
                            _pregnancies,
                            () => setState(() => _pregnancies++),
                            () => setState(() => _pregnancies--)),
                        _buildUniformCounter(
                            'ابن مفقود/غائب (يُحجز نصيبه)',
                            _missingSons,
                            () => setState(() => _missingSons++),
                            () => setState(() => _missingSons--)),
                        _buildUniformCounter(
                            'ابن خنثى مشكل (يعامل كأنثى احتياطاً)',
                            _hermaphroditeSons,
                            () => setState(() => _hermaphroditeSons++),
                            () => setState(() => _hermaphroditeSons--)),
                      ],
                    ),
                  ),
                  const Divider(height: 25),
                  _buildSectionCardHeader('الأصول (الآباء والأجداد والجدات)',
                      Icons.elderly_rounded, primaryNavy),
                  _buildUniformCounter(
                      'الأب',
                      _father,
                      () => setState(() => _father = 1),
                  ...
                  _buildUniformCounter(
                      'الجد الصحيح لأب',
                      _grandfather,
                      () => setState(() => _grandfather = 1),
                      () => setState(() => _grandfather = 0),
                      max: 1),
                  _buildUniformCounter(
                      'الجدة لأب (أم الأب)',
                      _paternalGrandmother,
                      () => setState(() => _paternalGrandmother = 1),
                      () => setState(() => _paternalGrandmother = 0),
                      max: 1),
                  _buildUniformCounter(
                      'الجدة لأم (أم الأم)',
                      _maternalGrandmother,
                      () => setState(() => _maternalGrandmother = 1),
                      () => setState(() => _maternalGrandmother = 0),
                      max: 1),
                  const Divider(height: 25),
                  _buildSectionCardHeader('الحواشي المقربة (الإخوة والأخوات)',
                      Icons.group_rounded, primaryNavy),
                  _buildUniformCounter(
                      'الإخوة الأشقاء',
                      _fullBrothers,
                      () => setState(() => _fullBrothers++),
                      () => setState(() => _fullBrothers--)),
                  _buildUniformCounter(
                      'الأخوات الشقيقات',
                      _fullSisters,
                      () => setState(() => _fullSisters++),
                      () => setState(() => _fullSisters--)),
                  _buildUniformCounter(
                      'الإخوة لأب',
                      _consanguineBrothers,
                      () => setState(() => _consanguineBrothers++),
                      () => setState(() => _consanguineBrothers--)),
                  _buildUniformCounter(
                      'الأخوات لأب',
                      _consanguineSisters,
                      () => setState(() => _consanguineSisters++),
                      () => setState(() => _consanguineSisters--)),
                  _buildUniformCounter(
                      'الإخوة والأخوات لأم',
                      _uterineSiblings,
                      () => setState(() => _uterineSiblings++),
                      () => setState(() => _uterineSiblings--)),
                  const SizedBox(height: 16),
                  Container(
                    padding: const EdgeInsets.all(12),
                    ...
                    child: Column(
                      ...
                      children: [
                        _buildSectionCardHeader(
                            'احتياط الحواشي (موقوف/مُجمد)',
                            Icons.hourglass_bottom_rounded,
                            Colors.blueGrey.shade800),
                        _buildUniformCounter(
                            'حمل (يرجى أن يكون أخ شقيق)',
                            _pregnantFullBros,
                            () => setState(() => _pregnantFullBros++),
                            () => setState(() => _pregnantFullBros--)),
                        _buildUniformCounter(
                            'أخ شقيق غائب/مفقود',
                            _missingFullBros,
                            () => setState(() => _missingFullBros++),
                            () => setState(() => _missingFullBros--)),
                        _buildUniformCounter(
                            'أخ شقيق خنثى (يعامل كأنثى)',
                            _hermaphroditeFullBros,
                            () => setState(() => _hermaphroditeFullBros++),
                            () => setState(() => _hermaphroditeFullBros--)),
                        const Divider(height: 15),
                        _buildUniformCounter(
                            'حمل (يرجى أن يكون أخ لأب)',
                            _pregnantConsBros,
                            () => setState(() => _pregnantConsBros++),
                            () => setState(() => _pregnantConsBros--)),
                        _buildUniformCounter(
                            'أخ لأب غائب/مفقود',
                            _missingConsBros,
                            () => setState(() => _missingConsBros++),
                            () => setState(() => _missingConsBros--)),
                        _buildUniformCounter(
                            'أخ لأب خنثى (يعامل كأنثى)',
                            _hermaphroditeConsBros,
                            () => setState(() => _hermaphroditeConsBros++),
                            () => setState(() => _hermaphroditeConsBros--)),
                        const Divider(height: 15),
                        _buildUniformCounter(
                            'حمل (يرجى أن يكون أخ لأم)',
                            _pregnantUterineBros,
                            () => setState(() => _pregnantUterineBros++),
                            () => setState(() => _pregnantUterineBros--)),
                        _buildUniformCounter(
                            'أخ لأم غائب/مفقود',
                            _missingUterineBros,
                            () => setState(() => _missingUterineBros++),
                            () => setState(() => _missingUterineBros--)),
                        _buildUniformCounter(
                            'أخ لأم خنثى (الذكر كالأنثى)',
                            _hermaphroditeUterineBros,
                            () => setState(() => _hermaphroditeUterineBros++),
                            () => setState(() => _hermaphroditeUterineBros--)),
                      ],
                    ),
                  ),
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildSectionCardHeader('امتداد العصبات وذوي الأرحام',
                            Icons.warning_amber_rounded, Colors.red.shade800),
                        _buildUniformCounter(
                            'أبناء الإخوة الذكور',
                            _nephews,
                            () => setState(() => _nephews++),
                        ...
                        _buildUniformCounter(
                            'الأعمام (أشقاء / لأب)',
                            _paternalUncles,
                            () => setState(() => _paternalUncles++),
                            () => setState(() => _paternalUncles--)),
                        _buildUniformCounter(
                            'أبناء الأعمام (ابن العم)',
                            _cousins,
                            () => setState(() => _cousins++),
                            () => setState(() => _cousins--)),
                        const Divider(height: 15),
                        _buildUniformCounter(
                            'العمات',
                            _paternalAunts,
                            () => setState(() => _paternalAunts++),
                            () => setState(() => _paternalAunts--)),
                        _buildUniformCounter(
                            'أبناء العمات',
                            _paternalAuntsOffspring,
                            () => setState(() => _paternalAuntsOffspring++),
                            () => setState(() => _paternalAuntsOffspring--)),
                        const Divider(height: 15),
                        _buildUniformCounter(
                            'الأخوال',
                            _maternalUncles,
                            () => setState(() => _maternalUncles++),
                            () => setState(() => _maternalUncles--)),
                        _buildUniformCounter(
                            'أبناء الأخوال',
                            _maternalUnclesOffspring,
                            () => setState(() => _maternalUnclesOffspring++),
                            () => setState(() => _maternalUnclesOffspring--)),
                        const Divider(height: 15),
                        _buildUniformCounter(
                            'الخالات',
                            _maternalAunts,
                            () => setState(() => _maternalAunts++),
                            () => setState(() => _maternalAunts--)),
                        _buildUniformCounter(
                            'أبناء الخالات',
                            _maternalAuntsOffspring,
                            () => setState(() => _maternalAuntsOffspring++),
                            () => setState(() => _maternalAuntsOffspring--)),
                      ],
                    ),
                  ),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('تشغيل محرك الفرز وحساب الأعيان',
                  style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: Colors.white,
                  ...
                  child: Column(
                    children: [
                      Row(textDirection: TextDirection.rtl, children: const [
                        ...
                        SizedBox(width: 10),
                        Text('جدول توزيع الأعيان والأصول الشرعية',
                            style: TextStyle(
                                color: Colors.white,
                                fontSize: 16,
                                fontWeight: FontWeight.bold,
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                              ...
                              label: const Text('طباعة التقارير',
                                  style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 12,
                                      fontFamily: 'Cairo'))),
          ...
          : Container(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                        'قم بإدخال الأعيان وتحديد حالات الاحتياط الفقهي لعرض التقرير.',
        ...
        child: Padding(
            ...
            child:
              ...
              Row(children: [
                Icon(
                    ...
                    color: isBlocked
                        ...
                        : (isContingency
                            ...
                            : (isWill
                                ? goldAccent
                                : (isRadd ? Colors.blue : Colors.green)))),
                const SizedBox(width: 8),
                Text(res['heir'],
                    style: const TextStyle(
                        fontWeight: FontWeight.bold,
                ...
                const Spacer(),
                if (!isBlocked)
                  Text('${(res['fraction'] * 100).toStringAsFixed(2)}%',
                      style: TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 16,
                          color: isContingency
              ...
              Text(
                  'الحالة الشرعية: ${res['status']} | ${res['reason'] ?? res['share']}',
                  style: TextStyle(
                      fontSize: 13,
              ...
              if (!isBlocked && res['breakdown'].toString().isNotEmpty) ...[
                ...
                Text(
                    isContingency
                        ? 'النصيب المُجمد والموقوف كأمانة:'
                        : 'النصيب المفرز من الأعيان والأصول:',
                    style: TextStyle(
                        fontSize: 13,
                        fontWeight: FontWeight.bold,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text('نظام تصفية التركات وتوزيع الأعيان',
              style:
