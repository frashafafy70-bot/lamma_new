...
import 'package:auto_route/auto_route.dart'; // 🟢 استدعاء auto_route
...
@RoutePage() // 🟢 إضافة الكلمة السحرية
...
class _TripChatPageState extends State<TripChatPage> {
  ...
  Future<void> _fetchOtherPartyInfo() async {
    final info =
        await context.read<TripChatCubit>().getOtherPartyInfo(widget.tripId);
  ...
  Future<void> _makePhoneCall(BuildContext context, String phoneNumber) async {
    if (phoneNumber.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('رقم هاتف الطرف الآخر غير متوفر حالياً',
        ...
        child: Directionality(
          ...
          child: Column(
            children: [
              Expanded(
                child: BlocConsumer<TripChatCubit, TripChatState>(
                    ...
                    if (state is TripChatLoaded) {
                      if (state.messages.length > _previousMessageCount) {
                        if (state.messages.isNotEmpty &&
                            state.messages.first.senderId != currentUserId) {
                  ...
                  buildWhen: (previous, current) {
                    return current is TripChatLoaded ||
                        current is TripChatError ||
                        current is TripChatLoading;
                  },
              ...
              ChatInputWidget(
                  tripId: widget.tripId, currentUserId: currentUserId),
            ],
          ),
        ),
      ),
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.warning_amber_rounded,
                color: AppColors.error, size: 70.sp),
            ...
            Text(errorMessage,
                ...
                style: TextStyle(
                    color: AppColors.textMuted.shade700, fontSize: 14.sp)),
