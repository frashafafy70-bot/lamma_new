...
class _PassengerRequestTabState extends State<PassengerRequestTab> with PassengerUIMixin {
  ...
  String _mapSelectionMode = AppConstants.mapModeNone; // ✅ استخدام الثوابت
  bool _isMapFullscreen = false;
  bool _isReverseGeocoding = false;

  final TextEditingController _mapSearchController = TextEditingController();
  List<PlaceSearchEntity> _placePredictions = [];
  final double _closeZoom = AppConstants.defaultMapZoom; // ✅ استخدام الثوابت
    ...
    if (_pickupLocation != null && _destinationLocation != null) {
      setState(() {
        _polylines.add(Polyline(
          polylineId: const PolylineId(AppConstants.tempRoutePolylineId), // ✅ استخدام الثوابت
  ...
  void _updateLocationOnMap(LatLng newLoc) {
    ...
    setState(() {
      ...
      if (_pickupLocation == null) {
        ...
        _markers.removeWhere((m) => m.markerId.value == AppConstants.pickupMarkerId); // ✅ استخدام الثوابت
        _markers.add(
          Marker(
            markerId: const MarkerId(AppConstants.pickupMarkerId), // ✅ استخدام الثوابت
            position: newLoc, 
            icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen), 
            infoWindow: InfoWindow(title: context.l10n.pickupLocation), 
          )
        );
        if (_mapSelectionMode != AppConstants.mapModeNone && _tempMapCenter == null) {
          _tempMapCenter = newLoc;
        }
        if (_mapController != null && _mapSelectionMode == AppConstants.mapModeNone) {
  ...
  void _openMapSelection(String mode) {
    ...
    setState(() {
      ...
      _tempMapCenter = mode == AppConstants.mapModePickup // ✅ استخدام الثوابت
          ? (_pickupLocation ?? const LatLng(AppConstants.fallbackLatitude, AppConstants.fallbackLongitude)) 
          : (_destinationLocation ?? _pickupLocation ?? const LatLng(AppConstants.fallbackLatitude, AppConstants.fallbackLongitude));
    });
  ...
  void _onMapTap(LatLng position) {
    if (_mapSelectionMode == AppConstants.mapModeNone) { // ✅ استخدام الثوابت
      setState(() {
        _isMapFullscreen = true;
        _mapSelectionMode = AppConstants.mapModePickup; // ✅ استخدام الثوابت
      });
    }
  ...
  void _confirmMapSelection() {
    ...
    setState(() {
      ...
      String locationText = (_mapSearchController.text.trim().isNotEmpty && _mapSearchController.text != context.l10n.locatingMap && _mapSearchController.text != context.l10n.fetchingAddress) 
          ? _mapSearchController.text.trim() 
          : "إحداثيات: ${finalLoc.latitude.toStringAsFixed(4)}, ${finalLoc.longitude.toStringAsFixed(4)}";

      if (_mapSelectionMode == AppConstants.mapModePickup) { // ✅ استخدام الثوابت
        _pickupLocation = finalLoc;
        _markers.removeWhere((m) => m.markerId.value == AppConstants.pickupMarkerId); // ✅ استخدام الثوابت
        _markers.add(
          Marker(markerId: const MarkerId(AppConstants.pickupMarkerId), position: finalLoc, icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen), infoWindow: InfoWindow(title: context.l10n.pickupLocation))
        );
        _formControllers.pickup.text = locationText;
      } else if (_mapSelectionMode == AppConstants.mapModeDestination) { // ✅ استخدام الثوابت
        _destinationLocation = finalLoc;
        _markers.removeWhere((m) => m.markerId.value == AppConstants.destinationMarkerId); // ✅ استخدام الثوابت
        _markers.add(
          Marker(markerId: const MarkerId(AppConstants.destinationMarkerId), position: finalLoc, icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRed), infoWindow: InfoWindow(title: context.l10n.destinationLocation))
        );
        _formControllers.destination.text = locationText;
      }
      _updateRoutePolyline();
      _mapSelectionMode = AppConstants.mapModeNone; // ✅ استخدام الثوابت
  ...
  void _submitTrip() {
    ...
    if (_pickupLocation == null) return showPassengerSnackBar(context.l10n.errorSelectPickup);
    
    if (_selectedCategory == TripCategory.internal || _selectedCategory == TripCategory.travel) {
      if (_destinationLocation == null) return showPassengerSnackBar(context.l10n.errorSelectDestination);
    } else if (_selectedCategory == TripCategory.shopping) {
      if (_formControllers.errandDetails.text.trim().isEmpty && _orderAudioFile == null) return showPassengerSnackBar(context.l10n.errorProvideOrderDetails);
    }

    if (_formControllers.price.text.trim().isEmpty) return showPassengerSnackBar(context.l10n.errorEnterPrice);

    double? parsedPrice = double.tryParse(_formControllers.price.text.trim());
    if (parsedPrice == null || parsedPrice <= 0) return showPassengerSnackBar(context.l10n.errorInvalidPrice);
  ...
  Widget _buildSelectedServiceForm(bool isSubmitting) {
    final Map<TripCategory, Widget Function()> formStrategies = {
      TripCategory.shopping: () => BuyOrdersServiceForm(
        ...
        primaryGreen: AppColors.primaryNavy,
      ...
      TripCategory.internal: () => RideServiceForm(
        vehicleType: _vehicleType,
        onVehicleChanged: (veh) { UXFeedback.selectionClick(); setState(() => _vehicleType = veh); },
        isSubmittingTrip: isSubmitting,
        pickupController: _formControllers.pickup,
        destinationController: _formControllers.destination,
        priceController: _formControllers.price,
    ...
    return BlocProvider.value(
      ...
      child: BlocConsumer<PassengerRequestCubit, PassengerRequestState>(
        listener: (context, state) {
          if (state is LocationLoading) setState(() => _isLoadingMap = true);
          else if (state is LocationLoaded) _updateLocationOnMap(state.position);
          else if (state is LocationError) { setState(() => _isLoadingMap = false); showPassengerSnackBar(state.message); }
          else if (state is LocationPermissionDenied) { setState(() => _isLoadingMap = false); showLocationPermissionDialog(context.l10n); }
          else if (state is AddressLoading) setState(() { _isReverseGeocoding = true; _mapSearchController.text = context.l10n.locatingMap; });
          else if (state is AddressLoaded) setState(() { _isReverseGeocoding = false; _mapSearchController.text = state.address; });
          else if (state is AddressError) setState(() { _isReverseGeocoding = false; _mapSearchController.text = state.message; });
          else if (state is PlacesSearchLoaded) setState(() => _placePredictions = List<PlaceSearchEntity>.from(state.predictions));
          ...
          else if (state is TripSubmitting) showTripLoadingDialog(context.l10n);
          else if (state is TripSubmitSuccess) {
            Navigator.of(context, rootNavigator: true).pop();
            UXFeedback.success(); 
            _formControllers.destination.clear(); _formControllers.price.clear(); _formControllers.errandDetails.clear(); _formControllers.errandEstimatedCost.clear(); _orderAudioFile = null;
            showPassengerSnackBar(context.l10n.requestSentSuccess, isError: false);
            widget.tabController.animateTo(2);
          }
          else if (state is TripSubmitError) { Navigator.of(context, rootNavigator: true).pop(); showPassengerSnackBar(state.message); }
          else if (state is RouteCoordinatesLoaded && state.routePoints.isNotEmpty && mounted) {
            setState(() {
              _polylines.removeWhere((p) => p.polylineId.value == AppConstants.tempRoutePolylineId); // ✅ استخدام الثوابت
              _polylines.add(
                Polyline(
                  polylineId: const PolylineId(AppConstants.finalRoutePolylineId), // ✅ استخدام الثوابت
        ...
        builder: (context, state) {
          bool isPickingMap = _mapSelectionMode != AppConstants.mapModeNone; // ✅ استخدام الثوابت

          return LayoutBuilder(
            builder: (context, constraints) {
              double actualContainerHeight = math.max(0.0, math.min(screenHeight * 0.55, constraints.maxHeight - keyboardHeight));
                ...
                children: [
                    ...
                    child: PassengerMapSection(
                      ...
                      onCameraMove: (pos) {
                        if (isPickingMap) {
                          ...
                          if (!_isReverseGeocoding) setState(() { _isReverseGeocoding = true; _mapSearchController.text = context.l10n.locatingMap; });
                        }
                      },
                      onCameraIdle: () => (isPickingMap && _tempMapCenter != null) ? _requestCubit.getAddressFromLatLng(_tempMapCenter!) : null,
                    ),
                  ),
                  ...
                  if (isPickingMap)
                    MapSelectionOverlay(
                      ...
                      onSelectPlace: (id, desc) => _requestCubit.fetchPlaceDetails(id, desc),
                      onCancel: () { 
                        UXFeedback.lightImpact();
                        setState(() { _mapSelectionMode = AppConstants.mapModeNone; _isMapFullscreen = false; _placePredictions = []; }); // ✅ استخدام الثوابت
                        FocusScope.of(context).unfocus(); 
                        _fitMapToMarkers(); 
                      },
                  ...
                  if (_isMapFullscreen && !isPickingMap)
                    Positioned(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(context.l10n.confirmLocation, style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.bold, fontSize: 16.sp)),
                      ),
                    ),
                  ...
                  PassengerActionPanel(
                    ...
                    serviceFormWidget: _buildSelectedServiceForm(state is TripSubmitting),
                  ),
                ],
              );
            },
