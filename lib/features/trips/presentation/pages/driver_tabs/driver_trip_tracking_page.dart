...
class _DriverTripTrackingView extends StatefulWidget {
  ...
  @override
  State<_DriverTripTrackingView> createState() => _DriverTripTrackingViewState();
}
...
class _DriverTripTrackingViewState extends State<_DriverTripTrackingView> {
  ...
  Future<void> _launchGoogleMapsNavigation(double? lat, double? lng) async {
    if (lat == null || lng == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('موقع العميل غير متوفر حالياً', style: TextStyle(fontFamily: 'Cairo', fontSize: 14.sp)), backgroundColor: AppColors.error),
        );
      }
      return; 
    }
    ...
    if (await canLaunchUrl(googleMapsUrl)) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('لا يمكن فتح خرائط جوجل', style: TextStyle(fontFamily: 'Cairo', fontSize: 14.sp)), backgroundColor: AppColors.error),
        );
      }
    }
  }
          ...
          body: StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              return Column(
                children: [
                  Container(
                    padding: EdgeInsets.only(top: MediaQuery.of(context).padding.top + 10.h, bottom: 16.h, left: 20.w, right: 20.w),
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [BoxShadow(color: AppColors.royalGreen.withValues(alpha: 0.25), blurRadius: 12, offset: const Offset(0, 4))],
                    ),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        InkWell(onTap: () => context.pop(), child: const Icon(Icons.arrow_forward_rounded, color: Colors.white, size: 28)),
                        Text('تتبع الرحلة', style: TextStyle(fontFamily: 'Cairo', fontSize: 20.sp, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 0.5)),
                        InkWell(
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.white,
                      boxShadow: const [BoxShadow(color: Color(0x14000000), blurRadius: 20, spreadRadius: 5, offset: Offset(0, -5))],
                      borderRadius: BorderRadius.vertical(top: Radius.circular(30.r)),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Center(child: Container(width: 40.w, height: 5.h, margin: EdgeInsets.only(bottom: 16.h), decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(10.r)))),
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Icon(Icons.location_on_rounded, color: AppColors.error, size: 24.sp),
                            SizedBox(width: 8.w),
                            Expanded(child: Text(destination, style: TextStyle(fontFamily: 'Cairo', fontWeight: FontWeight.bold, fontSize: 16.sp, color: AppColors.textDark), maxLines: 2, overflow: TextOverflow.ellipsis)),
                          ],
                        ),
                            ...
                            Expanded(
                              ...
                              child: SizedBox(
                                ...
                                child: ElevatedButton(
                                  style: ElevatedButton.styleFrom(backgroundColor: AppColors.info, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r))),
                                  onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => TripChatPage(tripId: widget.tripId))),
  ...
  Widget _buildDynamicActionButton(BuildContext context, String status) {
    if (status == 'accepted') {
      ...
      return ElevatedButton.icon(
        style: ElevatedButton.styleFrom(backgroundColor: Colors.grey, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r))),
