...
void initTripDriverDI() {
  // ==========================================
  // 1. Driver Repositories
  // ==========================================
  if (!sl.isRegistered<DriverRadarRepository>()) sl.registerLazySingleton<DriverRadarRepository>(() => DriverRadarRepositoryImpl(firestore: sl()));
  ...
  if (!sl.isRegistered<DriverLocationCubit>()) {
    sl.registerFactory(() => DriverLocationCubit(updateDriverLocationUseCase: sl()));
  }
  ...
  if (!sl.isRegistered<DriverHistoryCubit>()) {
    sl.registerFactory(() => DriverHistoryCubit(
          ...
          cancelTripUseCase: sl(), // من الـ Shared
          deleteTripUseCase: sl(), // من الـ Shared
        )); 
  }
}