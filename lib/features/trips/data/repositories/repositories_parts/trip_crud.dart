...
mixin TripCoreRepository on TripRepositoryBase {
  ...
  Future<Either<Failure, void>> updateTrip(TripEntity trip) async {
    try {
      ...
      await firestore.collection(collectionName).doc(trip.id).update(toModel(trip).toMap());
      return const Right(null);
    } catch (e) {
  ...
  Future<Either<Failure, String>> createPassengerTrip({
    ...
    try {
      ...
      DocumentReference docRef = await firestore.collection(collectionName).add(tripData);
  ...
  Future<Either<Failure, void>> submitTripRequest({
    ...
    try {
      ...
      DocumentSnapshot userDoc = await firestore.collection('users').doc(passengerId).get();
