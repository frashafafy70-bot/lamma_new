...
import 'passenger/available_travels_state.dart';
...
class AvailableTravelsCubit extends Cubit<AvailableTravelsState> {
  ...
  Future<void> _getPassengerLocation() async {
    ...
    if (!serviceEnabled) {
      _listenToTrips(); // كمل تحميل الرحلات حتى لو مفيش GPS
      return;
    }
    ...
    try {
      ...
      _listenToTrips(); // إعادة المعالجة بعد جلب الموقع
    } catch (e) {
      _listenToTrips();
    }
  }
  ...
  void toggleNearby(bool val) {
    ...
    _showOnlyNearby = val;
    // إعادة إرسال البيانات بناءً على الفلتر الجديد
  ...
  void _processAndEmitTrips(dynamic rawDocs, {bool forceReprocess = false}) {
    ...
    // دعم إعادة المعالجة من الذاكرة أو من الفايربيز
