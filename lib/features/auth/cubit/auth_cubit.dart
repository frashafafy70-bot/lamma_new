...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<void> login({required String email, required String password}) async {
    ...
    result.fold(
      ...
      (user) => emit(AuthSuccess('تم تسجيل الدخول بنجاح! 🎉',
  ...
  Future<void> sendSignUpOtp({required String phone}) async {
    ...
    if (!formattedPhoneNumber.startsWith('+20'))
      formattedPhoneNumber = '+20$formattedPhoneNumber';
  ...
  Future<void> verifyOtpAndCompleteSignUp({
    ...
    result.fold(
      ...
      (user) => emit(AuthSuccess('تم إنشاء وتفعيل حسابك بنجاح! 🎉🚀',
  ...
  Future<void> verifyOtp(
    ...
    result.fold((error) => emit(AuthError(error)), (userEntity) {
      if (userEntity != null) {
        emit(AuthSuccess('تم تسجيل الدخول بنجاح',
            uid: userEntity.uid, role: userEntity.role));
      } else {
  ...
  Future<void> verifyOtpAndResetPassword(
    ...
    final result = await authRepository.verifyOtpAndResetPassword(
        verificationId: verificationId,
        smsCode: smsCode,
