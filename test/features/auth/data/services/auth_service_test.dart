...
void main() {
  ...
  group('AuthService - loginUser Tests', () {
    ...
    test('يجب أن يسجل الدخول بنجاح، يحدّث الـ FCM Token، ويحفظ الـ Role في الكاش', () async {
      // Arrange (التجهيز)
      ...
      when(() => mockDocRef.update({'fcmToken': tToken})).thenAnswer((_) async => Future.value());
      ...
      // Act (التنفيذ)
      final result = await authService.loginUser(email: tEmail, password: tPassword);
    ...
    test('يجب أن يرمي FirebaseAuthException عند فشل تسجيل الدخول (مثلاً كلمة مرور خاطئة)', () async {
      // Arrange (التجهيز)
      ...
      expect(() => call(email: tEmail, password: tPassword), throwsA(isA<firebase_auth.FirebaseAuthException>()));
